...
double h_response(double t, double TT, double TLL, double SS) {
    return 0.5 * (erf(2 * sqrt(2) * t / (SS * TLL)) -
                  erf(2 * sqrt(2) * (t - TT) / (SS * TLL)));
}

double readback(double t, double jitter, Matrix *a, double SS, double TT,
                double TLL) {
    int len = a->column;
    double rs = 0;
    double tmp = 0;
    for (int i = 0; i < len; i++) {
        tmp =
            a->data[0][i] * h_response(t - (i + 1) * TLL + jitter, TT, TLL, SS);
        rs = rs + tmp;
    }
    return rs;
}
...
int compare(int _dataget_col, int _array_col,
            MATRIX_TYPE (*dataget)[_dataget_col],
            MATRIX_TYPE (*array)[_array_col]) {
    int i;
    for (i = 1; i <= 4; i++) {
        if (dataget[0][0] == array[0][i * 2 - 2] &&
            dataget[0][1] == array[0][i * 2 - 1]) {
            return i;
        }
    }
    return -1;
...
void encode_17pp(int _src_col, int _des_col, MATRIX_TYPE (*_src)[_src_col],
    ...
    while (_src_len > 0) {
        ...
        if (vssf == 4) {
            if (out_array[out_index][0] == 0 && out_array[out_index][1] == 1 &&
                out_array[out_index][2] == 0) {
    ...
    */
    ...
    for (int s = 0; s < numOfStates; ++s) {
        ...
        for (int b = 1; b <= 2; ++b) {
            ...
            trellis_pst[nextstate]->input[trellis_pst[nextstate]->counter - 1] =
                trellis_nst[cur]->input;
            trellis_pst[nextstate]
                ->output[trellis_pst[nextstate]->counter - 1] =
                trellis_nst[cur]->output;
            trellis_pst[nextstate]->pre[trellis_pst[nextstate]->counter - 1] =
                s;
    ...
    for (int i = 1; i < depth; ++i) {
        for (int j = 0; j < numOfStates; ++j) {
            for (int b = 1; b <= 2; ++b) {
                ...
                c = c * 1;
                state_value[b - 1] =
                    mat_state_metric->data[trellis_pst[j]->pre[b - 1]][i - 1] +
                    branch_metric;
            }
    ...
    for (int i = depth - 1; i >= 1; --i) {
        mat_detected_output->data[0][i - 1] =
            trellis_pst[state]->input[survivor_path[state][i - 1] - 1];
    ...
    /*map output (-1,1)->(0,1)*/
    mat_detected_output =
        M_nummul(M_numsub(mat_detected_output, -1.0), 0.5); /*(m+1)*2*/
